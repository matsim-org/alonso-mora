...
public class DiversionTest {
	...
	public void testRepeatedSameDestinationDiversions() {
		...
		controller.addOverridingQSimModule(new TestModeQSimModule(fleetSpecification));
	...
	static private class TestModeModule extends AbstractDvrpModeModule {
		...
		public void install() {
			...
			bindModal(DvrpLoadType.class).toInstance(new IntegerLoadType("passengers"));
		}
	}
	...
	static private class TestModeQSimModule extends AbstractDvrpModeQSimModule {
		...
		protected TestModeQSimModule(FleetSpecification fleetSpecification) {
			super(MODE);
			...
			if (vehicle.getSchedule().getStatus().equals(ScheduleStatus.STARTED)) {
				if (timer.getTimeOfDay() % DIVERSION_FREQUENCY == 0) {
					...
					if (currentTask instanceof DriveTask) {
						...
						VrpPathWithTravelData path = VrpPaths.calcAndCreatePathForDiversion(diversionPoint, endLink,
								router, travelTime);
			...
			if (vehicle.getSchedule().getStatus().equals(ScheduleStatus.STARTED)) {
				if (timer.getTimeOfDay() % DIVERSION_FREQUENCY == 0) {
					...
					if (currentTask instanceof DriveTask) {
						...
						// (2) Perform rerouting from the diversion point
						VrpPathWithTravelData path = VrpPaths.calcAndCreatePathForDiversion(diversionPoint,
								updatedEndLink, router, travelTime);
